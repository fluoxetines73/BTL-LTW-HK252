<div class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-4 border-bottom border-2 border-danger pb-2">
        <h2 class="text-uppercase text-danger fw-bold mb-0"><i class="fas fa-film me-2"></i>Phim Đang Chiếu</h2>
    </div>
    ...
    <div class="row">
        <div class="col-lg-3 mb-4">
            <div class="card shadow-sm border-0">
                ...
                <div class="card-body">
                    <form action="<?= BASE_URL ?>movies/current" method="GET">
                        <div class="mb-3">
                        ...
                        <div class="d-grid gap-2">
                            <button type="submit" class="btn btn-danger btn-sm fw-bold"><i class="fas fa-search me-1"></i> Lọc Phim</button>
                            <a href="<?= BASE_URL ?>movies/current" class="btn btn-outline-secondary btn-sm">Xóa bộ lọc</a>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div class="col-lg-9">
            <?php if (empty($nowShowing)): ?>
                <div class="alert alert-warning text-center py-5 shadow-sm border-0">
                    ...
                    <h5 class="text-muted">Không tìm thấy bộ phim nào phù hợp với điều kiện lọc.</h5>
                </div>
            <?php else: ?>
                            ...
                            <div class="card h-100 movie-card shadow-sm border-0 position-relative">
                                ...
                                <div class="card-body p-3 d-flex flex-column">
                                    ...
                                    <div class="mt-auto d-grid">
                                        <a href="<?= BASE_URL ?>product/detail/<?= $movie['id'] ?>" class="btn btn-danger btn-sm fw-bold">
                ...
                <?php if (isset($totalPages) && $totalPages > 1): ?>
                    <nav aria-label="Page navigation" class="mt-5">
                        <ul class="pagination justify-content-center">
                            ...
                            $baseLink = BASE_URL . 'movies/current?' . ($baseQueryString ? $baseQueryString . '&' : '');
                            ?>
                            ...
                            <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                                <li class="page-item <?= ($i == $currentPage) ? 'active' : '' ?>">
                                    <a class="page-link shadow-none <?= ($i == $currentPage) ? 'bg-danger border-danger text-white' : 'text-dark' ?>" 
